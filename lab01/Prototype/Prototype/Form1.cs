...
namespace Prototype
{
    public partial class Form1 : Form
    {
        ...
        // инициализируется в LoadPrototype(), вызываемом из конструктора.
        ...
        /// Создаёт надписи и числовые поля (созданные через вспомогательные методы),
        ...
        private void OnPropertyChanged(object sender, EventArgs e)
        {
            ...
            // Настройка прототипа происходит у верфи
        ...
        /// Кнопка Clone — клонирование (паттерн Прототип).
        ...
        /// Кнопка В бой! — запускает бой флота против случайных врагов.
        /// Отображаем вражеский флот визуально (карточки с HP/Shield)
        /// Отображаем бой раунд за раундом с задержкой между раундами
        /// Обновляем визуальные панели флотов после каждого раунда
        /// Обновляем флот — урон сохраняется, уничтоженные корабли удаляются
        private async void BtnBattle_Click(object sender, EventArgs e)
        {
            ...
            var battleLog = await battleEngine.RunBattleAsync(_fleet, enemyFleet, async (evt) =>
            {
                ...
                {
                    // Небольшая задержка между атаками (500 мс для видимости подсветки)
