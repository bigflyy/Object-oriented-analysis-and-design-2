...
Реализация без паттерна представлена в файле `ShipyardManual.cs` (также доступна в `WithoutPrototype/ShipyardManual.cs`). Можно подставить вместо `Shipyard` в `Form1.cs` и увидеть те же проблемы.
...
| Участник GoF | Класс в проекте | Роль |
|---|---|---|
...
3. **Глубокое копирование** — `Clone()` в каждом подклассе вызывает `Weapon.Clone()`, создавая независимую копию ссылочного типа `WeaponSystem`. Изменение оружия у клона не затрагивает оригинал.
...
│   └── Prototype/
│       ├── Models/
│       │   ├── Starship.cs          — абстрактный прототип
...
│       │   └── WeaponSystem.cs      — система вооружения (с Clone())
│       ├── UI/
│       │   ├── ShipRenderer.cs      — GDI+ отрисовка кораблей
│       │   ├── ShipCard.cs          — карточка корабля (HP/Shield бары)
│       │   └── FleetPanel.cs        — панель флота (список карточек)
│       ├── Shipyard.cs              — клиент паттерна (с Clone())
│       ├── ShipyardManual.cs        — реализация БЕЗ паттерна (с switch)
│       ├── BattleEngine.cs          — движок пошагового боя
│       ├── Form1.cs                 — логика формы
│       └── Form1.Designer.cs        — разметка UI
├── WithoutPrototype/
│   └── ShipyardManual.cs            — копия реализации без паттерна
└── READMEmy.md                      — этот файл
```

## Сборка и запуск

```bash
dotnet build lab01/Prototype/Prototype/Prototype.csproj
dotnet run --project lab01/Prototype/Prototype/Prototype.csproj
```
