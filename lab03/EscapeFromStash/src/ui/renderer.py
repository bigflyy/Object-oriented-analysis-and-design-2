...
class Renderer:
    ...
    def render(self, game: GameManager):
        ...
        stash_right = stash_x + game.stash.cols * self.CELL_SIZE + self.GRID_PADDING * 3
        ...
        self._render_log(game, stash_y + game.stash.rows * self.CELL_SIZE + self.GRID_PADDING * 2)
    ...
    def _render_drag_overlay(self, game: GameManager, x: int, y: int):
        ...
        for row in range(game.stash.rows):
            for col in range(game.stash.cols):
                if game.stash._can_place_at(item, col, row):
                    for dy in range(item.size_y):
                        for dx in range(item.size_x):
                            cr = row + dy
                            cc = col + dx
                            if 0 <= cr < game.stash.rows and 0 <= cc < game.stash.cols:
        ...
        # Draw image if available (same logic as in-grid rendering)
        img = self._get_image(item.name)
        if img is not None:
            if item.rotation == 0:
                target_w = int(item_rect.width * 0.85)
                target_h = int(item_rect.height * 0.85)
                scaled = pygame.transform.scale(img, (target_w, target_h))
            elif item.rotation == 180:
                target_w = int(item_rect.width * 0.85)
                target_h = int(item_rect.height * 0.85)
                ...
                pre_w = int(item.size_y * cell * 0.85)
                pre_h = int(item.size_x * cell * 0.85)
                scaled = pygame.transform.scale(img, (pre_w, pre_h))
                scaled = pygame.transform.rotate(scaled, -90)
            else:  # 270
                pre_w = int(item.size_y * cell * 0.85)
