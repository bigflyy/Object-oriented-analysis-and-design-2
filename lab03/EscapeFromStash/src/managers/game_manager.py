...
class Config:
    ...
    # === TIMING (seconds) ===
    TICK_INTERVAL = 2.0           # Main tick (hunger decay)
...
class GameManager:
    ...
    def update(self, dt: float):
        ...
        if self._tick_accumulator >= Config.TICK_INTERVAL:
            self._tick_accumulator = 0.0
            self._game_tick()
    ...
    def _try_loot_drop(self):
        ...
        if not self.stash.can_fit(new_item):
            self._end_game("Stash is full! No space for new loot!")
            return
        
        self.stash.add_item(new_item)
    ...
    def sell_item(self, item: Loot):
        """Sell an item using the SellVisitor."""
        item.accept(self.sell_visitor)
    
    def use_item(self, item: Loot):
        """Use an item using the UseVisitor."""
        item.accept(self.use_visitor)
    ...
    def process_removals(self):
        ...
        if self._pending_removal is not None:
            self.stash.remove_item(self._pending_removal)
    ...
    def fire_at_stash_item(self, target_item: Loot):
        ...
        if destroyed:
            self.stash.remove_item(target_item)
    ...
    def load_ammo_into(self, weapon: Weapon):
        ...
        self.use_visitor._load_ammo(weapon, self._ammo_selection)
    ...
    def upgrade_stash(self):
        ...
        if self.player.money >= Config.STASH_UPGRADE_COST:
            ...
            self.stash.upgrade(rows=Config.STASH_UPGRADE_ROWS)
